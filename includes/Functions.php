...
$REAL_URL = 'https://idealhomeinturkey.com';
$MAIN_URL = 'https://idealhomeinturkey.com';
...
function getHPP($box_id=0,$column){

	global $DB;

	$BoxInfo = $DB->query("SELECT * FROM ihit_homepage_projects WHERE box = '".$box_id."'");
	$BoxInfo = $BoxInfo->fetch(PDO::FETCH_ASSOC);

	return $BoxInfo[$column];
...
function getProjectType($ProjectID){
	...
	$ProjectTypeID = $DB->query("SELECT * FROM ihit_project_types_id WHERE project_id = '".$ProjectID."'");
	$ProjectTypeID = $ProjectTypeID->fetch(PDO::FETCH_ASSOC);
	$TypeID = $ProjectTypeID['type_id'];
	
	// Get Project Type Details
	$ProjectTypeDetails = $DB->query("SELECT * FROM ihit_projects_types WHERE id = '".$TypeID."'");
	$ProjectTypeDetails = $ProjectTypeDetails->fetch(PDO::FETCH_ASSOC);
	
	return $ProjectTypeDetails['title'];	
